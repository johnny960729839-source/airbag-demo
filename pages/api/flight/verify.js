...
export default async function handler(req, res) {
  try {
    ...
    const buildUrl = (params) => {
      ...
      u.searchParams.set('access_key', process.env.AVIATIONSTACK_KEY);
    ...
    const isIataForm = /^[A-Z]{2}\d+$/i.test(flight); // MU5101 这种
    ...
    // --- 3) 仍 0 条：若非 strict，则不带日期给前端返回最近样本 ---
    if (list.length === 0) {
      if (strict && date) {
